...
class MultiModalBuildingDetector(ImageBuildingDetector, LiDARBuildingDetector):
    def __init__(self, cfg):
        ...
        del self.lidar_backbone.head
        del self.image_backbone.head
        
        head_size = cfg.MODEL.HEAD_SIZE
        num_class = sum(sum(head_size, []))
        self.fusion_head = MultitaskHead(input_channels=cfg.MODEL.OUT_FEATURE_CHANNELS,num_class=num_class,head_size=head_size)
        
        self.image_lidar_fusion = self._make_conv(cfg.MODEL.OUT_FEATURE_CHANNELS+256,
                                                    1024,
    ...
    def forward_common(self, images, points, annotations):
        
        targets, _ = self.annotation_encoder(annotations)

        features = torch.cat((self.image_backbone(images), self.lidar_backbone(points)), axis=1)
        features = self.image_lidar_fusion(features)
        outputs = self.fusion_head(features)
    ...
    def forward_val(self, images, points, annotations):
        ...
        # TODO: besides the first line, the rest of the code is the same in all BuildingDetector classes, merge this code
